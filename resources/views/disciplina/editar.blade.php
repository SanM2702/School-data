    ...
    <div class="row">
        ...
        <div class="col-md-9 col-lg-10 p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="mb-0"><i class="fas fa-edit me-2"></i>Editar caso disciplinario</h4>
                <a href="{{ route('disciplina.mostrar', $disciplina->id) }}" class="btn btn-outline-secondary">
            ...
            <form method="POST" action="{{ route('disciplina.actualizar', $disciplina->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="row g-3">
                                ...
                                @if(!empty($canStaff))
                                    <select name="estado" class="form-select">
                                        @foreach(($estados ?? ['Registrado','Revisado','Sancionado','Cerrado','En apelación']) as $e)
                                            <option value="{{ $e }}" {{ $disciplina->estado === $e ? 'selected' : '' }}>{{ $e }}</option>
                                        @endforeach
                                    </select>
                                @else
                            ...
                            <div class="col-12">
                                ...
                                @if(!empty($canAcudiente))
                                    <textarea name="respuesta_acudiente" rows="3" class="form-control" placeholder="Respuesta del acudiente...">{{ old('respuesta_acudiente', $disciplina->respuesta_acudiente) }}</textarea>
                                @else
                            ...
                            <div class="col-12">
                                ...
                                @if(!empty($canStaff))
                                    <textarea name="observaciones" rows="3" class="form-control" placeholder="Observaciones...">{{ old('observaciones', $disciplina->observaciones) }}</textarea>
                                @else
                    ...
                    @if(!empty($canStaff) || !empty($canAcudiente))
